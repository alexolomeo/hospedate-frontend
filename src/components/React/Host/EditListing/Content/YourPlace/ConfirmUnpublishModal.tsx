...
export default function ConfirmUnpublishModal({
  ...
  const L =
    t.hostContent?.editListing?.commonComponents?.modal?.confirmUnpublish ?? {};
  ...
  const labelConfirm = L.confirm;
  const labelBack = L.back;
    ...
    <Modal
      ...
      topRightAction={
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-[16px] transition hover:bg-[var(--color-base-200-hover)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] disabled:opacity-60"
          disabled={busy}
        >
