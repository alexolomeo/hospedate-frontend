...
const ListingGrid = lazy(
  () => import('@/components/React/Listing/ListingsGrid')
);
const ListingMap = lazy(
  () => import('@/components/React/Listing/DisplayMap/ListingMap')
);
const FloatingActionButton = lazy(
  () => import('@/components/React/Common/FloatingActionButton')
);

// Lazy load icons
const MapIcon = lazy(() => import('/src/icons/map.svg?react'));
const ListIcon = lazy(() => import('/src/icons/list-bullet.svg?react'));
...
const ViewSwitcher: React.FC<Props> = ({ lang = 'es', initialQuery }) => {
  const [viewMode, setViewMode] = React.useState<0 | 1>(0);
  const toggleView = () => setViewMode((m) => (m === 0 ? 1 : 0));
  const [listings, setListings] = React.useState<PaginatedListings | null>(
    null
  );
  ...
  if (error) {
    return <div className="w-full px-4 py-12 text-center">{error}</div>;
  }
