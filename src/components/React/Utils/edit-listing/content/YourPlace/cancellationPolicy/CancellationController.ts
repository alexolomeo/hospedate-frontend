...
import type {
  CancellationForm,
  CancellationValidationMessages,
} from './cancellationValidators';
import { validateCancellationForm } from './cancellationValidators';
import { toCancellationPayload } from './cancellationAdapters';

type Watcher<T> = Set<(value: T) => void>;

export function createCancellationController(params: {
  slug: Slug;
  readOnly: boolean;
  form: CancellationForm;
  isDirty: () => boolean;
  reset: () => void;
  messages: CancellationValidationMessages;
  ...
  const validate = (): ControllerValidationResult => {
    ...
    const res = validateCancellationForm(form, messages);
    if (res.ok) {
  ...
  return {
    ...
    buildPatch() {
      return toCancellationPayload(form);
    },
