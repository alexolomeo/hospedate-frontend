...
} from './locationValidators';
import { validateLocationForm } from './locationValidators';
import { toLocationPayload } from './locationAdapters';

type Watcher<T> = Set<(v: T) => void>;

export function createLocationController(params: {
  slug: Slug;
  readOnly: boolean;
  form: LocationForm;
  isDirty: () => boolean;
  ...
  const validate = (): ControllerValidationResult => {
    ...
    const res = validateLocationForm(form, messages);
    if (res.ok) {
