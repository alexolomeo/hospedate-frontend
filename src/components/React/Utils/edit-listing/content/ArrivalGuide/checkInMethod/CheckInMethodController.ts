...
import type {
  CheckInMethodForm,
  CheckInMethodValidationMessages,
} from './checkInMethodValidators';
import { validateCheckInMethodForm } from './checkInMethodValidators';
import { toCheckInMethodPayload } from './checkInMethodAdapters';

type Watcher<T> = Set<(v: T) => void>;

export function createCheckInMethodController(params: {
  slug: Slug;
  readOnly: boolean;
  form: CheckInMethodForm;
  isDirty: () => boolean;
  reset: () => void;
  messages: CheckInMethodValidationMessages;
  ...
  const validate = (): ControllerValidationResult => {
    ...
    const res = validateCheckInMethodForm(form, messages);
    if (res.ok) {
  ...
  return {
    ...
    buildPatch() {
      return toCheckInMethodPayload(form);
    },
